...
summary: Defines ALA request execution, configured AchillesAgentLib execution, and authenticated coding-agent delegation with caller-owned mounts.
---
...
When at least one backend is detected, ALA must add its generic `coding-agent` Code Skill to the process-scoped discovery registry used by MainAgent. This internal skill is runtime infrastructure and is not a task skill. ALA mounts exactly the caller-supplied directories: the writable `--cwd <path> [as <alias>]` and each read-only `--folder <path> [as <alias>]`, or read-write with `write`. Without an alias a directory is mounted at its canonical absolute path; an alias mounts it under `/workspace/<alias>`. When `--cwd` is omitted ALA creates and retains a private temporary workspace. An explicit `--home` supplies persistent backend configuration and authentication state. ALA preserves the backend's native continuation inside a session and never deletes a caller-supplied or temporary working directory. The installed backend and runtime are mounted read-only and are never installed into the mounted directories. ALA must validate and mount the supplied directories without interpreting their configuration or communication protocols, and it must not discover, create, mount or overlay task skills. DS002 and DS004 define backend and mount details.

## Conclusion

ALA fulfills its purpose by accepting single-shot and interactive requests and running work through configured AchillesAgentLib components or a detected authenticated coding agent against caller-owned mounts while preserving clean output and bounded lifecycle behavior.
