...
| Authenticated coding-agent delegation | ALA detects installed agent CLIs, executes Anthropic task skills through them, and retains the selected agent and mounted skill catalog in a temporary workspace without requiring a direct provider key. |
...
When at least one backend is detected, ALA must add its generic `coding-agent` Code Skill to the process-scoped discovery registry used by MainAgent. This internal skill is runtime infrastructure and must not be counted as a task-specific task skill. ALA must create a private temporary workspace for the first delegated prompt, link every discovered Anthropic skill directory under `.agents/skills/<name>`, pass that directory instead of the caller's repository, preserve the backend's native continuation identifier, pin subsequent interactive prompts to the same backend, and remove the workspace at shutdown. Task-repository descriptors must never enter the AchillesAgentLib registry. These hidden boundaries let subscription-authenticated agents follow repository-owned skills and perform multi-step work without transferring repository ownership or credentials to ALA. DS002 and DS006 define backend and workspace details.
