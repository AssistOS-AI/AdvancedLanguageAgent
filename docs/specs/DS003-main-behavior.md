...
A calling application may replace repository discovery with `--skill-catalog <file>`, a JSON array of individual skill directory paths. ALA validates the listed SKILL.md descriptors and mounts only those directories read-only. The caller owns selection and removal of missing paths before continuation; ALA does not interpret skillset groups. See DS004 for manifest validation.
...
When at least one backend is detected, ALA must add its generic `coding-agent` Code Skill to the process-scoped discovery registry used by MainAgent. This internal skill is runtime infrastructure and must not be counted as a task-specific task skill. ALA uses an explicit `--cwd` as the persistent host workspace or creates a private temporary workspace when omitted, exposing it at `/workspace`. An explicit `--home` supplies persistent backend configuration and authentication state. Selected Anthropic skill directories are mounted strictly read-only under `/workspace/.agents/skills/<name>`. ALA introduces the mounted skills directory once when starting a native conversation, then sends subsequent task prompts without that prefix. ALA preserves the backend's native continuation inside a session and removes only temporary workspaces it created. The installed backend and runtime are mounted read-only and are never installed into the workspace. Caller-owned runtime dependencies may be supplied through repeatable read-only --folder <path> [as <alias>] mounts. ALA must validate and mount these directories without interpreting their configuration or communication protocols. DS002 and DS006 define backend and workspace details.
