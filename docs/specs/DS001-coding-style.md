...
AchillesAgentLib skill discovery must be limited to ALA-owned runtime skills such as the internal `coding-agent` Code Skill. Task-repository discovery must be implemented separately, must scan only files named `SKILL.md`, and must never reinterpret `cskill.md`, `oskill.md`, `tskill.md`, or `dcgskill.md` as task-skill descriptors. Anthropic task-skill execution must cross the internal coding-agent bridge and must not call an imported repository module as an AchillesAgentLib skill. An interactive task-repository refresh must preserve the active MainAgent, its AchillesAgentLib discovery registry, the coding-agent service, workspace files, selected backend, native continuation, and active folder registrations while replacing only the separate Anthropic catalog, symbolic router metadata, and strict read-only task-skill mount set for the next coding-agent process.
