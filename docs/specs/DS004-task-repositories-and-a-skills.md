...
ALA’s general skill prompt only directs the coding agent to use .agents/skills and read the relevant SKILL.md files. It must not enumerate skill names, descriptions or descriptor contents. Descriptor metadata remains available for validation and explicit selection. An explicit --skill must select that descriptor even when --ca chooses a backend; backend selection must not bypass explicit skill selection. Calling applications pass the request and selectors without duplicating ALA’s skill instructions.

Caller applications resolve described repository skillsets into concrete skill names and directories. ALA does not interpret robot skillset IDs. `--skill-catalog <file>` reads an exclusive caller-owned JSON array of absolute skill directory paths. ALA canonicalizes and deduplicates paths, requires exactly one valid SKILL.md directly in each listed directory, rejects nested descriptors and duplicate or reserved native names, and mounts each directory read-only under /workspace/.agents/skills/<name>. It rejects malformed manifests and missing paths with repository exit code 4. An empty array selects no task skills and never falls back to configured or environment repositories. ALA does not interpret repository skillsets, copy skill files, prune paths or rewrite the manifest; the caller owns selection, source lifetime and continuation preparation. `--skillSets` accepts exact skill names, including an explicit empty selection, and preserves that filter across repository refreshes. Either explicit selection activates a sandbox-local skill overlay; unselected host descriptors remain unchanged.
...
ALA must persist task repository paths in a versioned JSON configuration at `$HOME/.ala/config.json` by default. `ALA_CONFIG_PATH` must designate a root directory and move the configuration to `<ALA_CONFIG_PATH>/.ala/config.json`; it must not be interpreted as a JSON filename. An explicit `--config <file>` must retain the highest priority for one command. ALA must not import or fall back to the former `~/.config/ala/config.json` location. `ala repo add` and interactive `/repo add` must accept only a Git URL and reject local paths. The Git URL must map deterministically to a managed clone under `$XDG_DATA_HOME/ala/repositories`, falling back to `~/.local/share/ala/repositories`, and configuration must store its canonical local path without exposing embedded URL credentials. Local repository paths must enter discovery only through the platform-delimited `ALA_TASK_REPOSITORIES` environment value.
