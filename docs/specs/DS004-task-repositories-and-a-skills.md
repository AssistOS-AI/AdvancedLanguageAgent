...
An Anthropic task skill must own domain methodology and acceptance conditions. ALA must execute it only through a detected [coding agent](wiki.html#definition-coding-agent), mount its directory under `.agents/skills/<name>` in the temporary workspace, and allow the agent to resolve supporting resources relative to that directory.
...
ALA must persist task repository paths in a versioned JSON configuration at `$XDG_CONFIG_HOME/ala/config.json`, falling back to `~/.config/ala/config.json`. `--config` and `ALA_CONFIG_PATH` must be able to select another configuration file. `ala repo add` and interactive `/repo add` must accept only a Git URL and reject local paths. The Git URL must map deterministically to a managed clone under `$XDG_DATA_HOME/ala/repositories`, falling back to `~/.local/share/ala/repositories`, and configuration must store its canonical local path without exposing embedded URL credentials. Local repository paths must enter discovery only for an invocation through `--task-repo` or `ALA_TASK_REPOSITORIES`.
...
Interactive repository management must be local and must not submit its arguments or output to MainAgent or an LLM. After an addition or removal, ALA must resolve all persistent, environment-provided, and invocation-specific repositories again, validate the combined catalog, and expose that catalog to the next prompt in the same session. This refresh must leave the AchillesAgentLib discovery registry and MainAgent conversation active because task descriptors never enter that registry. It must close the previous coding-agent workspace and continuation before replacing the mounted skill set, preserve the symbolic-detection setting, and report the refreshed skill count. If combined validation fails, ALA must restore the previous persistent registration, leave the active catalog unchanged, and report the repository error locally.

ALA must combine persistent repositories with the platform-delimited `ALA_TASK_REPOSITORIES` value and repeated invocation-specific `--task-repo` options. It must deduplicate canonical paths, recursively discover files named exactly `SKILL.md`, reject empty repositories and duplicate task-skill names, and keep the combined catalog separate from the AchillesAgentLib [discovery registry](wiki.html#definition-discovery-registry). With a coding agent available, ALA must link the discovered skill directories into `.agents/skills` in that agent's temporary workspace. With no coding agent available, general MainAgent execution must remain available, while explicit `--skill` execution must fail with execution exit code `5`.
