...
- Create a private JSONL transcript under the active configuration directory's `sessions/` child only when an LLM prompt is actually submitted. Record submitted prompts, observable coding-agent messages and final output, overall results and errors; reduce AchillesAgentLib supervisor progress to the public `tool` and `reason` fields. Local configuration and catalog commands must not create transcripts.
