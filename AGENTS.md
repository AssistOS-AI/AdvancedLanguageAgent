...
This repository defines [Advanced Language Agent](docs/index.html) (ALA), a CLI that exposes generic language-task execution infrastructure. [AchillesAgentLib](docs/wiki.html#definition-achilles-agent-lib) is authorized for `MainAgent`, `LLMAgent`, model selection, sessions, and ALA's internal `coding-agent` Code Skill. ALA mounts exactly the directories a caller supplies through `--cwd` and `--folder`; it does not discover, create, mount or overlay task skills.
...
ALA distributes exactly one runtime skill: the generic `coding-agent` Code Skill that bridges `MainAgent` to Codex, OpenCode, or Pi. It is registered only when at least one backend is executable and is not a caller task skill. ALA implements no task catalog, repository manager, or symbolic router; callers own their own task methodology and directory layout.
...
- Mount semantics: `--cwd <path> [as <alias>]` is always writable, mounted at its canonical absolute path or at `/workspace/<alias>`; `--folder <path> [write] [as <alias>]` is read-only unless marked `write`. Sources are canonicalized; reserved destinations, duplicate cwd targets, and overlapping folders are rejected. A read-only ancestor may contain the writable cwd, so ancestors must be mounted before descendants. When `--cwd` is omitted ALA creates a private temporary directory and retains it after the runtime closes. ALA must not interpret mounted contents.
- Distinguish execution lifetimes precisely: one-shot mode creates a new runtime and, when coding-agent execution is used, mounts the explicit caller-owned cwd or a retained temporary directory, then closes without implicit cross-process conversation continuation; explicit --session-id with --resume-session retains a pinned native conversation; interactive mode retains one runtime, the lazily created working directory, pinned coding backend, and native continuation until session exit.
...
- Native coding-agent permission policy is selected with `--permissions ask-for-approval|full-access`, defaulting to full-access inside Bubblewrap. Codex controlled execution uses app-server requests; OpenCode owns an authenticated native server per execution and must never rewrite project configuration. Hosts answer advertised opaque choices through control stdin, not conversation prompts or a host approval cache. Pi supports full-access only and requires the verified 0.85.1-or-compatible RPC lifecycle. Preserve explicit denial, cancellation, and unsupported-protocol errors.
- Node and prefix-installed dependencies remain usable through narrow read-only runtime mounts; never expose an entire caller home to find an interpreter.
- External integrations use generic read-only `--folder` mounts. The caller prepares dependencies and configuration and owns any socket protocol and cleanup. ALA must not parse Ploinky configuration or add Ploinky-specific mounts.
- Keep interactive slash commands synchronized with the CLI: `/help`, `/agent`, `/agent help`, `/agent list`, `/agent <codex|opencode|pi> models`, `/agent <codex|opencode|pi> model <model-name|default>`, `/agent auto <prompt>`, `/agent <codex|opencode|pi> <prompt>`, `/websearch on|off`, `/permissions [mode]`, `/quit`, and `/exit` are local commands and must never be routed to an LLM.
...
- Caller task methods and their specifications stay in the directory that owns them. A downstream consumer must not copy them into this project's `docs/` tree.

## Runtime Defaults

Interactive `/permissions [ask-for-approval|full-access]` is a local command. It reports or updates the next execution's native policy without resetting the runtime or persisting configuration. It does not create an approval-response channel, and Pi must continue to reject ask-for-approval.

The executable is `ala`. User configuration resolves from an explicit `--config <file>` first; otherwise ALA treats `ALA_CONFIG_PATH` as a root directory, falls back to `$HOME`, and uses `<root>/.ala/config.json`. Legacy `taskRepositories` fields in existing configuration files are ignored. ALA does not store transcripts; explicit session mode stores native continuation metadata under the supplied home. While an interactive terminal waits for MainAgent or coding-agent execution, ALA must animate `Thinking.`, `Thinking..`, and `Thinking...` on the diagnostic stream, then clear the indicator before results or errors; non-terminal and single-shot output must remain unchanged.

The default route uses AchillesAgentLib `MainAgent` for general execution. Explicit `--ca`/`--agent` selection, or any active `--folder`, routes execution to the coding agent. ALA resolves the library from `--achilles-path` or `ACHILLES_AGENT_LIB_PATH`, then a sibling checkout, then the installed `ploinky-agent-lib` package.

At process startup, ALA detects Codex, OpenCode, and Pi through their binary override variables, standard installation locations, and `PATH`. When at least one is available, ALA registers its generic `coding-agent` Code Skill with MainAgent. `--agent` forces this execution path, `ala agent list` reports detection, and the default priority is Codex, OpenCode, then Pi. Interactive model-list commands must query the selected agent's native catalog adapter, and model-set commands must persist a backend-specific value under `codingAgents.models`. A configured value must be passed on every invocation of that backend; when no value is configured, ALA must omit the model option so the CLI uses its own default. LLMAgent model overrides must not be reinterpreted as coding-agent settings. Every coding-agent execution and native model-list process must run inside a fail-closed Linux Bubblewrap namespace. ALA must use the already installed backend through a read-only runtime bind and must never reinstall it. The launcher must clear inherited environment variables, reconstruct a backend-specific allowlist, and remount the sandbox root read-only. The caller's cwd and folders appear at their canonical paths, or under `/workspace/<alias>`, and unrelated host paths must remain absent; only controlled backend authentication/state paths may be mounted read-write. Explicit session mode persists backend and native continuation metadata under the supplied home, bound to the explicit cwd; it must fail when the exact saved conversation cannot be recovered. An interactive process must retain native continuation and the same working directory until session shutdown, and replace the `Thinking` indicator with supported live backend output when the first event arrives.

All direct model interactions owned by ALA must use AchillesAgentLib's `LLMAgent` configured through runtime configuration and environment values. Coding-agent interactions must use the internal Code Skill and the selected agent's native CLI. Manual configuration overrides must remain available.
...
- `src/` — CLI, configuration, AchillesAgentLib loading, coding-agent adapters, and runtime modules.
- `src/coding-agents/` — backend adapters, folder resolution, Bubblewrap sandbox construction, and the coding-agent service.
