...
- Distinguish execution lifetimes precisely: one-shot mode creates a new runtime and, when coding-agent execution is used, the explicit caller-owned cwd or a temporary workspace, then closes without implicit cross-process conversation continuation; explicit --session-id with --resume-session retains a pinned native conversation; interactive mode retains one runtime, lazily created workspace, pinned coding backend, and native continuation until session exit.
...
- Caller-owned runtime folders use explicit read-only --folder mounts. Preserve the isolated selected-skill overlay. Node and prefix-installed dependencies remain usable through narrow read-only runtime mounts; never expose an entire caller home to find an interpreter.
