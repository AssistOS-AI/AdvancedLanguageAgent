...
- Caller-owned script capabilities may be mounted only through the explicit canonical `--runtime-bridge` directory at `/run/ala-runtime`. Preserve its read-only boundary and bridge-opt-in filtered skill overlay without modifying host skill descriptors. Node and prefix-installed package dependencies must remain usable through narrow read-only runtime mounts; never expose an entire caller home to find an interpreter.
- Direct Ploinky skill scripts instead opt into `--ploinky-task`: SDK and identity configuration are read-only, only its task-receipt `events` directory is writable. This explicitly grants the caller's MCP identity to the native agent; no socket or MCP proxy is involved. Preserve the filtered skill overlay and reject missing or noncanonical context paths.
