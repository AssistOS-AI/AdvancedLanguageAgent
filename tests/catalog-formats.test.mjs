...
test('explicit skill takes priority with a caller catalog, backend and generic folder', async t => {
  const root = await fixture(t);
  const selected = await writeAnthropicSkill(root, 'selected');
  const other = await writeAnthropicSkill(root, 'other');
  const file = join(root, 'catalog.json');
  await writeFile(file, JSON.stringify([selected, other]));
  const calls = [];
  const events = [];
  const runtime = await createRuntime({ achillesModule: { discoverSkills() {}, MainAgent: class {
    async buildSkills() {}
    async executeSkill(name, prompt) { calls.push({ name, prompt }); return prompt; }
    shutdown() {}
  } }, repositories: await readSkillCatalog(file),
  codingAgents: [{ name: 'codex', available: true, binary: '/fake/codex' }],
  options: { agent: 'codex', skill: 'selected', skillCatalog: file,
    folders: [{ source: root, alias: 'caller-runtime' }], tags: [] },
  diagnostics: captureStream(), eventSink: event => events.push(event) });
  t.after(() => runtime.close());
  await runtime.execute('Continue');
  assert.equal(calls[0].name, 'coding-agent');
  assert.match(calls[0].prompt, /skill "selected"/);
  assert.match(calls[0].prompt, /Read \.agents\/skills\/selected\/SKILL.md again/);
  assert.equal(calls[0].prompt.split('User request:').length, 2);
  assert.doesNotMatch(calls[0].prompt, /other|selected test skill|revision|Available skills:/);
  const catalogs = events.filter(event => event.type === 'skill-catalog');
  assert.equal(catalogs.length, 1);
  assert.deepEqual(catalogs[0].entries.map(entry => entry.name).sort(), ['other', 'selected']);
});
