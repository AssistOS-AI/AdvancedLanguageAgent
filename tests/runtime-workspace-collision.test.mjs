...
for (const local of [false, true]) {
  ...
  test(`runs a ${layout} installed below /workspace across the task workspace overlay`, {
    ...
    for (const folder of [pkg, dependency, path.join(root, 'task/.agents'), path.join(root, 'home')]) {
...
console.log(JSON.stringify({ dependency, writable, cwd: process.cwd(), home: process.env.HOME,
...
console.log(JSON.stringify(results));`;
    ...
    assert.deepEqual(fs.readdirSync(path.join(root, 'task')).sort(), ['.agents', 'result.txt']);
