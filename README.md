...
When a prompt actually reaches an LLM, ALA creates a private JSONL transcript under the active `.ala/sessions/` directory. The default layout is `$HOME/.ala/config.json` plus `$HOME/.ala/sessions/`; when `ALA_CONFIG_PATH` names another root, both paths move under `<ALA_CONFIG_PATH>/.ala/`. An explicit `--config <file>` places `sessions/` beside that file. Each one-shot invocation gets one transcript, while all prompt turns in one interactive invocation share one transcript. It contains submitted prompts, observable Codex, Pi, or OpenCode progress and final messages, the normalized ALA result, errors, and lifecycle events. AchillesAgentLib progress records contain only the selected tool name and its public `reason` message. Local commands such as repository, model, web-search, and folder configuration do not create a transcript unless the same invocation also executes a prompt. Files and the directory use owner-only permissions. A forceful external termination such as `SIGKILL` cannot run ALA's finalizer, so the already appended JSONL records remain but the file may lack its terminal event.
